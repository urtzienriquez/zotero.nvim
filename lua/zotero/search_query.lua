...
--   a OR b, a | b  either (binds tighter than the implicit AND:
...
--   author:"a OR b c"
--                  inside a prefixed quote, OR / | separate alternatives
--                  (here "a" or the phrase "b c"); without one it is a
--                  single phrase
--
-- Anything that doesn't parse as an operator (a lone OR, a bare `-`, an
-- unbalanced quote, an empty `note:`) is searched for literally.
--
...
-- one or more values, any of which may match:
--   { { { scope = "author", negate = false, values = { { text = "huey", phrase = false } } }, ... }, ... }
local M = {}
...
local function to_term(tok)
  ...
  if inner and #rest >= 2 then
    ...
    term.values = term.scope == "meta" and { { text = inner, phrase = true } } or quoted_values(inner)
  else
    term.values = { { text = rest, phrase = false } }
  end
  return term
end
...
function M.parse(str)
  ...
  for idx, tok in ipairs(tokens) do
    ...
    if is_or(tok) and #groups > 0 and not pending_or and tokens[idx + 1] and not is_or(tokens[idx + 1]) then
      pending_or = true
    else
