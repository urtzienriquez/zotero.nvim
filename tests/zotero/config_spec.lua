...
describe("config.set / config.get", function()
  ...
  it("defaults to the OS opener and no hidden item types", function()
    ...
    assert.same({}, config.get().hidden_item_types)
    assert.equals("<leader>zT", config.get().keymaps.items_filter_type)
    assert.equals("compact", config.get().feed_view)
    assert.equals("<leader>zR", config.get().keymaps.items_toggle_read)
  end)
